...
test.describe("Responsive Design Tests", () => {
  ...
  viewports.forEach(({ name, width, height }) => {
    test(`should display sign-in page correctly on ${name} (${width}x${height})`, async ({
      page,
    }) => {
    ...
    test(`should display dashboard correctly on ${name} (${width}x${height})`, async ({
      page,
    }) => {
      await page.setViewportSize({ width, height })
  ...
  test("should have touch-friendly buttons on mobile devices", async ({
    page,
  }) => {
    ...
    for (const deviceName of devices) {
      ...
      const context = await browser.newContext(
        typeof device === "object" && device !== null && !Array.isArray(device)
          ? (device as BrowserContextOptions)
          : {}
      )
  ...
  test("should maintain aspect ratios for images and icons", async ({
    page,
  }) => {
    await page.goto("/")
